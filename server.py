...
def client_handler(connection):
    conn = sqlite3.connect("chat.db")  # Conectar ao banco de dados localmente na thread
            ...
            if "action" in data:
                if action == "singup":
                    ...
                    if sender_id:
                        ...
                        # Get recipient ID
                        cursor.execute(
                            "SELECT id FROM users WHERE username = ?", (destination,)
                        )
                        ...
                        if recipient_id:
                            ...
                            sql, params = receive_msg(
                                sender_id, message, recipient_id, signature, public_key
                            )
                            cursor.execute(sql, params)
                            conn.commit()
                            connection.send(
